...
function showOptions() {
  setTimeout(() => {
    document.getElementById("vyberOtazku").style.display = "block";
  }, 600);
}
...
function saveResult(user, test, points, passed) {
  if (loggedUser != "") {
    ...
    xhttp.open(
      "POST",
      "http://localhost:8080/testy/webapi/resources/saveResult",
      true
    );
...
function getOneQuestion(i) {
  ...
  clearInterval(time);
  document.getElementById("timer").innerHTML = "20m:00s";
  let adr = "http://localhost:8080/testy/webapi/resources/getQuestion";
  let method = "POST";
  let obj;
  let xhttp = new XMLHttpRequest();
...
function checkCookies() {
  let adr = "http://localhost:8080/testy/webapi/resources/listCookies";
  let method = "GET";
  let str = "";
...
function login() {
  let method = "POST";
  let adr = "http://localhost:8080/testy/webapi/resources/userLogin";
...
function logout() {
  ...
  let adr = "http://localhost:8080/testy/webapi/resources/userLogout";
  let method = "GET";
  let str = "";
...
function eval() {
  let hodnotenie = "";
  if (test != 0) {
    /* 1-12 za 2 body; 16-23 za 2 body; 24-27 za 3 body	*/ // 13,14,15 za 1 bod
...
function timer() {
  let distance = countDownDate - new Date().getTime(); // ziskame aktualny cas
  let minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60)); // vypocitame aktualne minuty
  let seconds = Math.floor((distance % (1000 * 60)) / 1000); // //vypocitame aktualne  sekundy
...
function getTest(n) {
  ...
  adr = "http://localhost:8080/testy/webapi/resources/getTest";
  method = "POST";
...
function displayTest(obj) {
  ...
  for (let k in obj) {
    ...
    if (i >= 16) {
      ...
      if (i >= 24) newImg.setAttribute("class", "crossroad");
      else newImg.setAttribute("class", "imgs");
...
function logAnswer() {
  ...
  let cisloOtazky = "";
  let l = 0;
  for (l = 0; l < id.length - 2; l++) {
    // zisti cislo otazky , na kt som odpovedal
    cisloOtazky = "";
    if (id.charAt(l) == "_") {
...
function checkIfAnswered(q, o, id) {
  ...
  if (odpovede[q - 1] == "") {
    ...
    document.getElementById(test + "_" + q + odpovede[q - 1]).style.color =
      "#B3F2FF"; // starej odpovedi dame povodnu farbu
    document.getElementById(test + "_" + q + o).style.color = "#19FF19"; // novej odpovedi dame zelenu farbu
    odpovede[q - 1] = o;
  }
}
...
function displayQ(obj, i) {
  ...
  if (obj.otazka_id > 27)
    file =
      "images/" +
      Math.floor(obj.otazka_id / 27 + 1) +
      "_" +
      (obj.otazka_id % 27) +
      ".png";
  // cisloTestu_cisloOtazky.png
  else file = "images/" + 1 + "_" + obj.otazka_id + ".png";
  if (obj.otazka_id % 27 == 0)
    file = "images/" + Math.floor(obj.otazka_id / 27 + 1) + "_27.png";
  // "next" button
  newDiv = document.createElement("div");
  ...
  if (
    (obj.otazka_id >= 16 && obj.otazka_id <= 27) ||
    obj.otazka_id % 27 >= 16 ||
    obj.otazka_id % 27 == 0
  ) {
    let newImg = document.createElement("img");
...
function nextq() {
  ...
  if (tOtazky.length == 0)
    document.getElementById("data").innerHTML = "dosli otazky :(";
}
